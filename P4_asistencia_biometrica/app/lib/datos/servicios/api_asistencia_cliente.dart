...
import '../../core/red/interceptor_auth.dart';
import '../local/almacen_seguro.dart';

class ApiAsistenciaCliente {
  ApiAsistenciaCliente({String? baseUrl}) {
    final base = baseUrl ?? const String.fromEnvironment(
      'BACKEND_URL',
    ...
    // Inyectar el interceptor de autenticación con cola
    _dio.interceptors.add(InterceptorAuth(_almacen, _dioLimpio));
  }

  late final Dio _dio;
  late final Dio _dioLimpio;
  final AlmacenSeguro _almacen = AlmacenSeguro();
  ...
  Future<Response<dynamic>> postPublico(String path, Map<String, dynamic> data) async {
    try {
      return await _dioLimpio.post(path, data: data);
    } on DioException catch (e) {
      throw _mapError(e);
    } on SocketException {
      throw Exception('Sin conexión a la red');
